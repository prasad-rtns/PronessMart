...
class CartQueries {
   ...
   */
  // cartQueries.js — replace the findByUserId implementation with:
  ...
  async addItem(userId, productData, quantity = 1, sessionId = null) {
    try {
      ...
      // 1) create if missing atomically
  ...
  async addMultipleItems(userId, items, sessionId = null) {
    try {
      ...
      // 1️⃣ Atomically create cart if it doesn’t exist
      const cart = await Cart.findOneAndUpdate(
        query,
        { $setOnInsert: doc },
        { new: true, upsert: true, setDefaultsOnInsert: true }
  ...
  async syncCartWithProducts(userId) {
    try {
      ...
      return await this.findByUserOrSessionF(userId);
    } catch (error) {
  ...
  async getAllCarts(filters = {}, page = 1, limit = 20, sort = "-createdAt") {
    ...
    try {
      ...
      const queryFilters = { ...filters, status: "active" };
      ...
      if (filters?.category) {
        ...
        delete queryFilters.category; // Remove from top level
      }
      ...
      if (filters?.sku) {
        ...
        delete queryFilters.sku; // Remove from top level
      }
      ...
      if (filters?.search) {
        ...
        delete queryFilters.search; // Remove from top level
      }
      ...
      if (filters?.name) {
        ...
        delete queryFilters.name; // Remove from top level
      }
      ...
      if (filters?.brand) {
        ...
        delete queryFilters.brand; // Remove from top level
      }
