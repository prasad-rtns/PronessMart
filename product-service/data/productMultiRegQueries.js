...
class ProductMultiRegQueries {
  ...
  async getProducts(filters = {}) {
    try {
      ...
      logger.error(`query.category: ${query.category}`);
  ...
  async getProductsByCategorySlug(categorySlug, options = {}) {
    try {
      ...
      logger.info(`products by categorySlug: ${categorySlug}`);
      return this.getProducts({
        ...options,
        category: categorySlug
      });
    } catch (error) {
      logger.error(`Error getting products by category Code: ${error.message}`);
      throw error;
    }
  }
